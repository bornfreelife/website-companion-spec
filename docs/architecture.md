...
A publisher may select from supported components and provide content, bounded theme tokens, static assets, schedules, and external links. Theme tokens may include a bounded `groupAccentPalette`; clients map those colors onto their own fixed group components and do not accept publisher CSS. Schedule resources may declare routine points, bounded relationships from planner time groups to those points, step visibility, user-confirmed sequential navigation, and an optional bounded transition from the end of one visible track to another declared local choice. Catalogues may declare bounded, data-only regional fallback order and opt into fixed client-owned local order tracking. A publisher cannot add executable behavior or a new native capability without a client/store release.
