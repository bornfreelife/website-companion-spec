...
A publisher may select from supported components and provide content, theme tokens, static assets, schedules, and external links. Schedule resources may declare routine points and bounded relationships from planner time groups to those points; the user supplies local times and explicitly enables reminders. A publisher cannot add executable behavior or a new native capability without a client/store release.
